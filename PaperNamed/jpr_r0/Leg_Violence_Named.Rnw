...
<<models, include=FALSE>>=
    ...
    vars <- c("country", "year", "violence", "system", "DemAge", "maj", "MajCat",
            "govfrac", "singleParty", "pr", "tenshort", "UDS", "polity2",
            "ethnicAlesina", "CWtrust", "higherTrust", "CWsurvSelfExpr",
            "legislature", "elect_legislature", "disproportionality", "gini",
            "GDPperCapita", "enps", "enpv", "federal", "immunity", "liec",
            "UNMurderRate", "WomenInParl")
    ...
    leg.raw$MajCat <- factor(leg.raw$MajCat, labels = c("Minority", "Regular Maj.",
                                                        "Strong Maj."))
    ...
    ## Only Countries with Elected Legislatures after 1989 (new sample 1990)
...
<<OutTable2, warning=FALSE, echo=FALSE, message=FALSE, results='asis'>>=
    ...
    ModNamesB <- c("A1", "A2", "A3", "A4", "A5", "A6", "A7", "A8")

    texreg(list(DN1, DN2, DN3, DN4, DN5, DN6, DN7, DN8),
        table = FALSE,
        custom.coef.names = CoefNames,
        custom.model.names = ModNamesB,
        use.packages = FALSE)
@
}
\end{center}
...
<<predProb, warning=FALSE, echo=FALSE, message=FALSE, results='hide', fig.width=7, fig.height=4, out.width='0.8\\linewidth', cache=TRUE>>=
  ## Create Graphs ##
