...
In order to \remove{further} systematically explore the causes of legislative violence across the contemporary globe, I used keyword searches of the Google News Archive \add{LexisNexis, NewsLibrary, NewsBank, general Google Search, and YouTube} to create a data set of {\emph{physical fights between legislators in national legislative chambers}}. \add{See the Online Appendix for details.} \change{This search was}{These searches were} supplemented by information from colleagues resulting in a data set of \change{91}{131} incidents of legislative violence\add{, 86 of which were} in \change{32}{30} \change{countries with elected multi-party national legislative chambers}{democracies} between 1981 and \change{Winter 2011}{2012}.
...
\noindent \add{Full replication files--including data and source code--for the empirical analysis in this article can be found at }
%\url{https://github.com/christophergandrud/leg_violence_paper1}
\add{[URL WITHHELD FOR BLIND REVIEW]. All analyses were conducted using R} \citep{R-cite}.
