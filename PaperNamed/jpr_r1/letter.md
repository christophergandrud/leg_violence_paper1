...
I am hesitant to overly interpret such a result in the article if published. As discussed on page 21, it is well-established in the statistical literature that it can be difficult to find meaningful interactions in data sets with relatively small numbers of events. As such, more research is likely needed in the future when longer-time series become available. However, it should be noted that the weak evidence that does exist suggests that an interactive effec would not be contrary to, but instead would strengthen support for the theory presented in the paper.
